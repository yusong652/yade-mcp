"""Tests for wait_for_background_run — the post-exec O.wait() that
...
from yade_mcp_bridge.runtime.background_run import mark_background_run
...
class TestWaitForBackgroundRun:
    def test_waits_when_running_true(self, fake_yade, runner, scratch_script):
        ...
        mark_background_run(True)  # as if the script called O.run(wait=False)
...
def _cleanup_signals():
    ...
    reset or a prior test's mark_background_run(True) would leak."""
    mark_background_run(False)
